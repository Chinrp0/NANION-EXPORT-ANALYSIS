function test_final_validation()
    ...
    try
        ...
        fprintf('Wells passed filtering: %d (%.1f%%)\n', ...
            filteredData.numWellsPassed, 100*filteredData.numWellsPassed/extractedData.numWells);
        
        % Show filter report details
        filterReport = filteredData.filterReport;
        fprintf('\nFilter Failures:\n');
        fprintf('  Series R > 50 MΩ: %d wells\n', filterReport.seriesFailures.count);
        fprintf('  Seal R > 50 GΩ: %d wells\n', filterReport.sealFailures.count);
        fprintf('  Capacitance > 250 pF: %d wells\n', filterReport.capacitanceFailures.count);
        ...
        if filteredData.numWellsPassed > 0
            ...
            fprintf('Well ID   | Series R (MΩ) | Seal R (GΩ) | Cap (pF) | Current (pA)\n');
            fprintf('----------|---------------|-------------|----------|-------------\n');
            
            for i = 1:numSamples
                fprintf('%-8s | %12.1f | %10.2f | %7.1f | %10.1f\n', ...
